...
4. 点「保存」。插件会在后台拉取列表并下载图片，之后每 6 小时检查一次新图；Muzei 把现有图片轮完时也会触发刷新

设置页也可以从桌面图标直接打开。改了方向或轮换范围后，旧图会被移出轮换池。

### 显示方式

Muzei 自己的铺法是按高度铺满屏幕，比屏幕宽的部分按桌面报告的翻页位置平移。这个位置只有桌面（launcher）能提供：很多桌面已经不再报告（或只有一页），Muzei 就一直停在初始位置（图片最左边，或桌面固定报告的正中间），插件也收不到翻页信息，无法代替桌面实现平移。设置页的「显示方式」可以改为由插件按手机屏幕分辨率先排好，再交给 Muzei：

| 方式 | 效果 | 位置滑条 |
| --- | --- | --- |
| 不处理（默认） | Muzei 原本的效果；桌面支持时左右随翻页平移，否则停在最左边或正中间 | 不起作用 |
...
每次刷新列表后，插件会把图片池里本地还没有的原图**预先下载**到自己的私有目录，永久保存、不自动清理。Muzei 换图时插件只从本地读，不在换图的那一刻联网；只有某张图还没来得及下载时才临时下载一次。

- 刷新列表：保存设置时、Muzei 请求加载且列表超过 1 小时没刷新时，以及每 6 小时一次
- 缺的图超过 50 张（例如第一次选「全部」）时只在 Wi-Fi 下预下载，平时的几张新图任何网络都会下载
- 系统限制后台任务单次最多 10 分钟，大批量下载会分几轮续传
- Muzei 自带的缓存放在系统缓存目录，可能被系统或清理软件清掉，也最多只留最近显示过的 100 张；有了本地原图，这些都不影响换图，改显示方式时也直接用本地原图重新排版

### 网络不稳时

- 某张图从 Worker 下载失败后，接下来 30 秒内本地没有原图的图片都直接失败，不再逐张等待连接超时（网络不通时 Muzei 会把图片池里的图挨个试一遍）
- 拉取列表失败时，从 30 秒起线性递增重试，最多 5 次；之后交给 Muzei 下一次请求加载或 6 小时的定时刷新
